...
const Deals = () => {
  ...
  const resetCountdown = (
    ...
    const newDurationMs =
      (newHours * 3600 + newMinutes * 60 + newSeconds) * 1000;
  ...
  return (
    <section className="lg:mx-[80px] px-4 sm:px-8 my-16 sm:my-32">
      <div className="container mx-auto flex flex-col">
                ...
                <div>
                  ...
                  <div className="h-48 overflow-hidden relative">
                    ...
                    <div className="flex items-end absolute overflow-hidden inset-0 bg-gradient-to-t from-black/60 to-transparent duration-300  opacity-0 group-hover:opacity-100 group-active:opacity-100">
                      <p
                        className={`${deal.highlightColor} text-xs w-fit py-1 px-2 m-4 rounded-full text-white `}
                      >
                        {deal.highlight}
                      </p>
                    </div>
                  ...
                  <div className="p-4">
                    ...
                    <p className="flex items-center gap-2 text-purple-600 group-hover:underline group-active:underline">
                      Shop Now{" "}
                      <span className="transition-all group-hover:translate-x-1 group-active:translate-x-1">
                        <deal.Icon />
                      </span>{" "}
                    </p>
                  </div>
          ...
          <div className="sm:w-[90%] mx-auto">
            <div className=" flex sm:flex-row flex-col items-center justify-between gap-3 mx-auto w-full max-w-5xl mt-12 md:p-6 p-4 bg-gradient-to-r from-purple-600/20 to-blue-600/20 rounded-3xl duration-100 hover:shadow-xl active:shadow-xl backdrop-blur-md border border-white/50 group">
              <div className="flex flex-col max-md:items-center">
                <p className="px-3 py-1 mb-3 text-sm bg-purple-100 text-purple-700 font-medium rounded-full w-fit">
                  Flash Sale
                </p>
                ...
                <Button className="button gradient md:self-start group-hover:-translate-y-1 group-active:-translate-y-1">
                  {" "}
                  Shop the Sale{" "}
              ...
              <div className="flex gap-3 justify-center md:mt-4 mt-2">
                {countDown.map((time, index) => (
                  <div
                    className="bg-white/70 p-1 h-16 w-16 backdrop-blur-md rounded-full flex flex-col items-center justify-center border border-white/50"
                    key={index}
                  >
        ...
        <Fade direction="up" delay={500}  triggerOnce className="mt-10 self-center">
          <Button className="text-black bg-t-white button">
            {" "}
            More Deals{" "}
            <span>
              {" "}
          ...
          {resetButton && (
            <Button
              variant="ghost"
              className="self-center button mt-10 "
              onClick={() => resetCountdown(hours, minutes, seconds)}
            >
              Reset to {hours} hours
            </Button>
          )}
        </Fade>
      </div>
